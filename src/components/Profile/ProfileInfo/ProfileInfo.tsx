...
function ProfileInfo(props: profile) {
    return (
        <div>
            ...
            {
                props.isFetch ? <Preloader/> : <div className={s.descriptionBlock} >
                    <img className={s.avatar}
                         src={props.profile.photos.large == null ? userIcon : props.profile.photos.large} alt="..."/>
                    <div><span>Full name: </span>{props.profile.fullName}</div>
                    <div><span>About me:</span> {props.profile.aboutMe == null ? "..." : props.profile.aboutMe}</div>
                    <div className={s.jobBlock}>
                        {props.profile.lookingForAJob ? <div><span>I`m looking for a job</span></div> : "I have job"}
